...
X,y = np.genfromtxt('./1-Regression/data1.txt', delimiter=',').T    #Loading training samples
m=len(y)    #Num of training samples
y=np.reshape(y,(m,1));X=np.reshape(X,(m,1))       #Check X,y is in shape (m,1) not (m,)

plotScatter(X,y);plt.show()       #Plotting training data
...
plotLine(X[:,1],np.matmul(X,theta));plotScatter(X[:,1],y);plt.show()        #Plotting the fitted regression
